...
<script>
  ...
  import CardRecountItemsTable from "../../../components/Cards/CardRecountItemsTable";
    ...
    created() {
      ...
      this.getRecountById({id: this.id});
    },

    methods: {
      ...mapActions('recount', ['getRecountById', 'updateRecount']),
      ...mapActions('shop', ['setDialogVisibilityShop', 'handleSelectShop']),
      ...mapActions('depot', ['setDialogVisibleDepot', 'handleSelectDepot']),

      handleSubmit() {
        const shopID = (this.selectedShop.id) ? this.selectedShop.id : this.recount.shopid;
        const depotID = (this.selectedDepot.id) ? this.selectedDepot.id : this.recount.depotid;
        if (shopID && depotID) {
          this.updateRecount({id: this.id, shopID, depotID});
        }
      },
