...
<template>
    ...
    <a-row type="flex" :gutter="24" justify="space-around" align="middle">
      ...
      <a-col :span="24" :md="12" :lg="{span: 12, offset: 0}" :xl="{span: 6, offset: 0}" class="col-form">
        <h1 class="mb-15">{{$t('create_receipt.title')}}</h1>
        <h5 class="font-regular text-muted">{{ $t('create_receipt.text_message') }}</h5>
        ...
        >
          <a-form-item class="mb-10" :label="$t('create_receipt.product')" :colon="false">
            <a-button type="primary" @click="showModal">{{ $t('create_receipt.select_product_button') }}</a-button>
            <div class="author-info">
              {{this.selectedProductName}}
            </div>
          </a-form-item>
          <a-form-item class="mb-5" :label="$t('create_receipt.quantity')" :colon="false">
            <a-input
                v-decorator="[
						'quantity',
                ...
                type="number"
                :placeholder="$t('create_receipt.quantity')"
                v-model="quantity"/>
          </a-form-item>
          <a-form-item class="mb-5" :label="$t('create_receipt.cell_number')" :colon="false">
            <a-input
                v-decorator="[
						'cell_number',
                ...
                type="text"
                :placeholder="$t('create_receipt.cell_number')"
                v-model="cell_number"/>
          </a-form-item>
          <a-form-item>
            <a-button type="primary" block html-type="submit" class="login-form-button">
              {{ $t('create_receipt.receipt_button') }}
            </a-button>
          </a-form-item>
        </a-form>
        <!-- / Sign In Form -->

        <p class="font-semibold text-muted">{{ $t('create_receipt.back_to_receipts') }} <router-link to="/receipts" class="font-bold text-dark">
          {{ $t('create_receipt.back') }}</router-link></p>
      </a-col>
    ...
    methods: {
      ...
      handleSubmit() {
        ...
        if (product && quantity && cell_number) {
          this.createReceipt({ user, product, quantity, cell_number });
        }
      },
