...
<template>
    ...
    <a-row type="flex" :gutter="24" justify="space-around" align="middle">
      ...
      <a-col :span="24" :md="12" :lg="{span: 12, offset: 0}" :xl="{span: 6, offset: 0}" class="col-form">
        <h1 class="mb-15">Edit product receipt</h1>
        <h5 class="font-regular text-muted">Edit product receipt</h5>
        ...
        >
          <a-form-item class="mb-10" label="Product" :colon="false">
            <a-input v-model="receipt.product.name" :disabled="true"></a-input>
          </a-form-item>
...
<script>
  ...
  import {mapActions, mapState} from "vuex";
    ...
    methods: {
      ...mapActions('receipt', ['updateReceipt', 'getReceiptById']),
      handleSubmit() {
        const product = this.receipt.product._id;
        const {
            _id: id,
            quantity,
            cell_number,
        ...
        if (id && product && quantity && cell_number) {
          this.updateReceipt({ id, product, quantity, cell_number });
        }
      },
