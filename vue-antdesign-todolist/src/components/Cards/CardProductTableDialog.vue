<template>
	...
	<a-card :bordered="false" class="header-solid h-full" :bodyStyle="{padding: 0,}">
        ...
        <a-col :span="24" :md="10">
          <a-input-search class="header-search" :class="searchLoading ? 'loading' : ''" placeholder="Type here…" @search="handleFindProduct" v-model="keyword" :loading='searchLoading'>
...
<!--              <path fill-rule="evenodd" clip-rule="evenodd" d="M8 4C5.79086 4 4 5.79086 4 8C4 10.2091 5.79086 12 8 12C10.2091 12 12 10.2091 12 8C12 5.79086 10.2091 4 8 4ZM2 8C2 4.68629 4.68629 2 8 2C11.3137 2 14 4.68629 14 8C14 9.29583 13.5892 10.4957 12.8907 11.4765L17.7071 16.2929C18.0976 16.6834 18.0976 17.3166 17.7071 17.7071C17.3166 18.0976 16.6834 18.0976 16.2929 17.7071L11.4765 12.8907C10.4957 13.5892 9.29583 14 8 14C4.68629 14 2 11.3137 2 8Z" fill="#111827"/>-->
      ...
      <template slot="slctBtn" slot-scope="row">
        <a-button v-on:click="handleSelectProductItem(row._id, row.name)" :data-id="row._id"  class="btn-edit">
          SELECT
        </a-button>
      </template>
...
<script>
  ...
  import {mapActions, mapState} from "vuex";
    ...
    computed: {
		  ...mapState('product', ['current', 'pageSize', 'current', 'totalCount'])
    },

    methods: {
      ...mapActions('product', ['getAllProducts', 'getProductsByKeyword', 'handleSelectProduct', 'setCurrent', 'setPageSize', 'setDialogVisibility']),
      ...
      handleSelectProductItem(id, name){
        this.handleSelectProduct({id, name});
