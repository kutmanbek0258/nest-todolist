<template>
	...
	<a-card :bordered="false" class="header-solid h-full" :bodyStyle="{padding: 0,}">
      ...
      <template slot="editBtn" slot-scope="row">
        <a-button v-on:click="openLink('/references/update-supplier/' + row.id)" :data-id="row.id"  class="btn-edit">
          Edit
        </a-button>
      </template>
...
<script>
  ...
  import {mapActions, mapState} from "vuex";
    ...
    computed: {
		  ...mapState('supplier', ['current', 'pageSize', 'totalCount'])
    },

    methods: {
      ...mapActions('supplier', ['getAllSuppliers', 'deleteSupplier']),
